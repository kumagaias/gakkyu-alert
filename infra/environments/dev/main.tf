...
module "amplify" {
  ...
  environment_variables = {
    EXPO_PUBLIC_API_BASE_URL = var.custom_domain != null ? "https://api.${var.custom_domain}" : "https://${aws_api_gateway_rest_api.api.id}.execute-api.${var.aws_region}.amazonaws.com/${var.environment}"
  }
}
...
# ---------------------------------------------------------------------------
# API Gateway カスタムドメイン (api.<custom_domain>)
# ---------------------------------------------------------------------------
