...
function createMcpServer(req: Request): McpServer {
  const server = new McpServer({ name: "gakkyu-alert-fhir", version: "1.0.0" });
  for (const tool of tools) tool.registerTool(server, req);
  return server;
}
...
app.get("/healthz", (_req, res) => {
  res.json({
    status: "ok",
    server: "gakkyu-alert-fhir-mcp",
    synthetic: process.env.SYNTHETIC_MODE === "true",
  });
});
...
// SSE transport (Prompt Opinion / legacy compatibility)
