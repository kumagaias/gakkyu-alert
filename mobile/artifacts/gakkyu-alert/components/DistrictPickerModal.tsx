...
import {
  resolveDistrictByGps,
  resolveDistrictByZip,
  getDistrictGroups,
} from "@/utils/location";
...
export function DistrictPickerModal({ visible, title, selectedId, onClose, onSelect }: Props) {
  ...
  const [resolved, setResolved]               = useState<District | null>(null);
  const [showList, setShowList]               = useState(false);

  const groups = getDistrictGroups();
  ...
  const handleGps = async () => {
    ...
    if (result.type === "success") {
      setResolved(result.district);
    } else {
      setGpsError(GPS_ERROR_MESSAGES[result.type] ?? "エラーが発生しました");
    }
  };
  ...
  const handleZip = async () => {
    ...
    if (result.type === "success") {
      ...
      setZipError(ZIP_ERROR_MESSAGES[result.type] ?? "エラーが発生しました");
    }
  };
  ...
  const handleManualSelect = (d: District) => {
    onSelect(d);
    onClose();
  };
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        {showList ? (
                ...
                {group.data.map((d, i) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      selectedId === d.id && { backgroundColor: colors.accent },
                    ]}
                    onPress={() => handleManualSelect(d)}
                    activeOpacity={0.7}
                  >
                    <Text style={[styles.districtName, { color: selectedId === d.id ? colors.primary : colors.foreground }]}>
                      {d.name}
                    </Text>
                    {selectedId === d.id && (
          ...
          <View style={[styles.searchArea, { paddingBottom: insets.bottom + 20 }]}>
            ...
            {gpsError && (
              <Text style={[styles.errorText, { color: colors.destructive }]}>{gpsError}</Text>
            )}
            ...
            <View style={styles.zipRow}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  {
                    backgroundColor: colors.primary,
                    opacity: zipLoading || zipCode.replace(/\D/g, "").length !== 7 ? 0.45 : 1,
                  },
                ]}
                onPress={handleZip}
              ...
              >
                {zipLoading ? (
                  ...
                  <Text style={styles.zipBtnText}>検索</Text>
                )}
              </TouchableOpacity>
            </View>
            {zipError && (
            ...
            {/* Resolved district — confirm card */}
            {resolved && (
              <View style={styles.resolvedArea}>
                <View style={[styles.resolvedCard, { backgroundColor: colors.accent, borderColor: colors.border }]}>
                  ...
                  <Text style={[styles.resolvedLabel, { color: colors.primary }]}>
                    {resolved.name}が見つかりました
                  </Text>
                </View>
                <TouchableOpacity
