...
export const registerDevice = async (
  ...
  return customFetch<DeviceRegistrationResult>(getRegisterDeviceUrl(), {
    ...options,
    method: "POST",
 ...
 * @summary デバイス解除
 */
export const getDeregisterDeviceUrl = (fcmToken: string) => {
  return `/api/v1/devices/${fcmToken}`;
};
...
export const deregisterDevice = async (
  ...
  return customFetch<void>(getDeregisterDeviceUrl(fcmToken), {
    ...options,
    method: "DELETE",
  });
};
...
>(options?: {
  ...
  TContext
> => {
  const mutationKey = ["deregisterDevice"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return deregisterDevice(fcmToken, requestOptions);
  };
...
export type DeregisterDeviceMutationError = ErrorType<unknown>;
...
>(options?: {
  ...
  TContext
> => {
  return useMutation(getDeregisterDeviceMutationOptions(options));
};
